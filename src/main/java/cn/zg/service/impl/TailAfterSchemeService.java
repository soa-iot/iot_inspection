...
import cn.zg.dao.inter.ValueLRDDLDao;
...
import cn.zg.service.inter.TailAfterSchemeInter;
 ...
 */
@Service
public class TailAfterSchemeService implements TailAfterSchemeInter{
	private static Logger logger = LoggerFactory.getLogger( TailAfterSchemeService.class );
	...
	@Autowired
	private  ValueLRDDLDao  valueLRUDDLDao;
...
//				System.out.println(list);
	 ...
	 */
	@Override
	public List<Map<String,Object>> getInspectioNames(String userId){
		logger.debug( "---S--------获取所有的方案信息 ---------" ); 
		try {
			List<Map<String,Object>> list = valueLRUDDLDao.findInspectionNames(userId);
			ArrayList<String> ids = new ArrayList<String>();
			for( Map<String,Object> m : list ) {
				ids.add( (String)m.get("NAME") );
			}
			List<Map<String, Object>> infos = valueLRUDDLDao.findInspectionIdsNames(ids);

layui.use(['form', 'laypage', 'table', 'laydate', 'layer'], function() {
	...
	// 监听工具条
	table.on('toolbar(flash_steam_record_table)', function(obj) { // 注：tool是工具条事件名，test是table原始容器的属性
				var layEvent = obj.event;
				if (layEvent === 'save') { // 保存

					flash_steam_record_table.config.LAY_CHECKED = true;
	...
	table.on('edit(flash_steam_record_table)', function(obj) { // 注：edit是固定事件名，test是table原始容器的属性
				// lay-filter="对应的值"
	...
	// 监听行工具事件
	table.on('tool(flash_steam_record_table)', function(obj) {
				var data = obj.data;
				// console.log(obj)
				if (obj.event === 'add') {
					var add_data = {};
					add_data.ID = data.ID;
					add_data.PARA_ID = data.PARA_ID;
					add_data.PARA_NAME_CN = data.PARA_NAME_CN;
					add_data.RECORD_NUM = guid();
					// console.log(add_data);
					var table_data = table.cache[flash_steam_record_table.config.id];
					table_data.push(add_data);
					// console.log(table_data);
					table.reload('flash_steam_record_table', {
								data : table_data,
								url : ''
							});
				}
	...
	function loadTable(beginDate, endDate) {
		...
		cols[0].push({
					title : '操作',
					toolbar : '#cols_toolbar',
					width : 80,
					fixed : 'right'
				});
		// console.log(cols);
		flash_steam_record_table = table.render({
			elem : '#flash_steam_record_table',
			...
			parseData : function(res) { // res 即为原始返回的数据
				...
				$.each(data, function(index, item) {
					if (formatdata[item.PARA_NAME_CN + item.RECORD_NUM]) {
						// 此条数据存在
						...
						data_item.min = item.PARA_VALUE;
						data_item.max = item.PARA_VALUE;
						formatdata[item.PARA_NAME_CN + item.RECORD_NUM] = data_item;
					}
				});
					...
					for (key in item) {
						...
						if (key.match(reg)) {
							if (item[key]) {
								...
								format_data_item.RECORD_NUM = item.RECORD_NUM;

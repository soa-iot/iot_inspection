layui.use(['form', 'laypage', 'table', 'laydate', 'layer'], function() {
	...
	// 监听工具条
	table.on('toolbar(solution_record_table)', function(obj) { // 注：tool是工具条事件名，test是table原始容器的属性
				var layEvent = obj.event;
				if (layEvent === 'save') { // 保存

					solution_record_table.config.LAY_CHECKED = true;
	...
	table.on('edit(solution_record_table)', function(obj) { // 注：edit是固定事件名，test是table原始容器的属性
				// lay-filter="对应的值"

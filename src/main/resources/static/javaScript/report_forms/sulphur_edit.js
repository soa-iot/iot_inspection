layui.use(['form', 'laypage', 'table', 'laydate', 'layer'], function() {
	...
	// 监听工具条
	table.on('toolbar(sulphur_record_table)', function(obj) { // 注：tool是工具条事件名，test是table原始容器的属性
				var layEvent = obj.event;
				if (layEvent === 'save') { // 保存

					sulphur_record_table.config.LAY_CHECKED = true;
	...
	table.on('edit(sulphur_record_table)', function(obj) { // 注：edit是固定事件名，test是table原始容器的属性
				// lay-filter="对应的值"
				obj.tr.find('td[data-field ="' + obj.field + '"]').css('color',
						'blue');
				console.log(obj);
				if (isNaN(obj.value) && obj.data.PARA_NAME_CN != '外观') {
	...
	function loadTable(beginDate, endDate) {
		...
		// console.log(cols);
		sulphur_record_table = table.render({
					elem : '#sulphur_record_table',
					...
					parseData : function(res) { // res 即为原始返回的数据
						...
						$.each(data, function(index, item) {
									if (formatdata[item.PARA_NAME_CN]) {
										...
										data_item.INDICATOR = item.INDICATOR;

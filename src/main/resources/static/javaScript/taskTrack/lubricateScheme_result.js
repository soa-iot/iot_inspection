...
var laydate = layui.laydate
	...
	,tableDataUrl = '/iot_inspection/productionReport/tailAfterScheme/lubstatus'//ipPort + 
	,inspectionUrl = '/iot_inspection/productionReport/tailAfterScheme/inspectionNames'//ipPort + 
	,staticTitlePart1 = "任务跟踪"
	,staticFlag = " - "
	,staticTitle =staticFlag +  staticTitlePart1
	,currentTableHeadTitle = ""
	,paramJson = {
		time : nowDateStr.split( ' ' )[0].replace( /\//g , '-' ).substring(0,7),
		currentScheme : ""
	}
	,tableObj;
...
//		    		 '</div>',
		    ...
		    cols : [[
		    	{type:'numbers',width:'10%', title: '序号'}
			      ...
			      ,{field:'remark2', width:'10%', title: '任务时间',align:'center'}
...
//	var tr = obj.tr; //获得当前行 tr 的DOM对象
	...
	if(layEvent === 'detail'){ //查看
		console.log( paramJson );
		console.log( obj.data.record_TIME.substring(0,10) );
		
		//跳转
		location.href = ipPort + "/html/productionReport/lubricate/lubricate_result.html?"
			+ "time=" + obj.data.record_TIME.substring(0,10) + "&scheme=" + paramJson.currentScheme;		
	} else {
...
function searchPlan( jsonData ){
	...
	if( jsonData.state == 0 ){
		if( jsonData.data != null){
			//排序
			var orderedInspectName = [];
			
				$.each( jsonData.data, function( index1, item1 ){	
					//console.log(item1.SCHEME_NAME);
					if(item1.SCHEME_NAME.indexOf("润滑") != -1){
						var tempInspect = {};

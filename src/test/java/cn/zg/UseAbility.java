...
public class UseAbility {
	...
	public void setRequireId() {
		String baseSql = "  from CZ_INSPECTION_PLAN ip"
				+ "  		right JOIN cz_inspection_scheme isc on ip.SCHEME_ID = isc.SCHEME_ID AND  isc.scheme_type = '1'"
		...
		String sql = " select ip.plan_id,ip.plan_name,isc.SCHEME_ID,isc.SCHEME_NAMe,ipo.POINT_ID,ipo.POINT_NAME,"
				...
				+ " 			  ire.EQU_POSITION_NUM,ire.REQUIRE_UNIT," + "             (CASE ire.REQUIRE_SYMBOL\r\n"
				+ "			  WHEN 'ba' THEN ire.REQUIRE_MIX || '~' || ire.REQUIRE_MAX \r\n"
				+ "			  WHEN 'ge' THEN '≥' || ire.REQUIRE_MIX"
				+ "			  WHEN 'gt' THEN '>' || ire.REQUIRE_MIX"
				+ "			  WHEN 'le' THEN '≤' || ire.REQUIRE_MAX"
				+ "			  WHEN 'lt' THEN '<' || ire.REQUIRE_MAX" + " END ) as DATARANGE";
		...
		for (int i = 0; i < oldConfig.size(); i++) {
			...
			if (pointname != null) {
				for (int j = 0; j < oldConfig.size(); j++) {
					Map<String, Object> tempMap1 = oldConfig.get(j);
					if (tempMap1.get("CONTENT_NAME") != null
							&& tempMap1.get("CONTENT_NAME").toString().equals(projectname1)) {
						secondColspan1++;
					}
				}
			} else {
			...
			// 生成合并单元格PROJECTNAME2
			String projectname2 = tempMap.get("REQUIRE_CONTEXT") == null ? ""
					: tempMap.get("REQUIRE_CONTEXT").toString();
			// 生成合并单元格SECONDCOLSPAN2
			int secondColspan2 = 1;

			// 生成位号POSITIONNUM
			String positionnum = tempMap.get("EQU_POSITION_NUM") == null ? ""
					: tempMap.get("EQU_POSITION_NUM").toString();
			// 生成单位UNIT
			...
			String tempSql = " insert into CZ_INSPECTION_HEADCONFIG " + " values ( '" + id + "','" + planid + "','"
					+ pointname + "','" + firstColspan + "','" + projectname1 + "','" + secondColspan1 + "','"
					+ projectname2 + "','" + secondColspan2 + "','" + positionnum + "','" + unit + "','" + fourColspan
					+ "','" + dataRange + "','" + fiveColspan + "','" + requireid + "' ) ";
